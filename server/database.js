...
exports.validateOneTimeToken = function(token, callback) {
    assert(token);
    // if ( token == "9ea411b9-7c55-4be0-a4be-ea9420ee8cef") {
    //     callback(null, {
    //         id: 100000000,
    //         username: "guest",
    //         balance_satoshis: 10000000000
    //     })
    // } else {
    //     callback("ott token does not exist", null)
    // }
      ...
      client.query('UPDATE games SET ended = true WHERE id = $1', [gameId],
        function (err) {
          ...
          client.query(endGameQuery, [userIds, playIds, bonusesAmounts], function(err, result) {
            ...
            wallet.updateBalanceArray(bonuses, function(err, res) {
                console.log("wallet api update Balance result <----", err, res);
            });
            callback();
          });
