...
Chat.prototype.join = function(io, socket, socketUser, channelName) {
    var self = this;

    self.io = io;
    var moderator = socketUser && socketUser.moderator;
...
Chat.prototype.onSay = function(socket, socketUser, message, isBot, customChannelName) {
    if (!socketUser)
    ...
    if (cmdMatch) //If the message is a command try to execute it
        this.doChatCommand(socketUser, cmdMatch, channelName, socket);
    else //If not broadcast the message
        this.say(socket, socketUser, message, channelName, isBot);
...
Chat.prototype.say = function(socket, user, message, channelName, isBot) {
    ...
    if (utils.hasOwnProperty(self.muted, user.username)) {
        ...
        if (muted.end < date) {
            ...
            self.sendMessageToUser(socket, {
                ...
                username:  user.username,
                role:      user.userclass,
                message:   'You\'re muted. ' + utils.printTimeString(muted.end - date) + ' remaining',
                bot:       false,
                channelName: channelName
            });
            return;
    ...
    self.sendMessageToChannel(socket, channelName, msg, user.id);
};
...
Chat.prototype.sendMessageToChannel = function(socket, channelName, msg, userID) {
...
Chat.prototype.mute = function(shadow, moderatorUser, username, time, channelName, callback) {
    var self = this;
    var now = new Date();
    var ms  = utils.parseTimeString(time);
...
Chat.prototype.unmute = function(moderatorUser, username, channelName, callback) {
    ...
    if (!utils.hasOwnProperty(self.muted, username))
