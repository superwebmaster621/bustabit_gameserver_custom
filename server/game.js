...
function Game(lastGameId, lastHash, bankroll, gameHistory) {
    ...
    function runGame() {
        ...
        db.createGame(self.gameId + 1, function (err, info) {
            ...
            wallet.getBankroll(function(err, res) {
                if (err) {
                    console.log('get bankroll from wallet service');
                    self.maxWin = Math.round(self.bankroll * 0.03); // Risk 3% per game
                }
                else 
                    self.maxWin = Math.round(parseFloat(res) * 0.03);
    ...
    function startGame() {
        ...
        for (var i = 0; i < arr.length; ++i) {
            var a = arr[i];
            bets[a.user.username] = {
                 bet: a.bet,
                 autoCashOut: a.autoCashOut
            };
    ...
    function endGame(forced) {
        ...
        if (self.crashPoint !== 0) {
            bonuses = calcBonuses(self.players, self.crashPoint);
    ...
    self.cacheCashoutTasks.push(function(cb) {
        db.cashOut(play.user.id, play.playId, cashed, function(err) {
            if (err) {
                ...
                self.emit('cashed_out', {username: play.user.username, stopped_at: at});
...
function calcBonuses(input, crashPoint) {
    ...
    slideSameStoppedAt(sorted,
        function(listOfRecords, cashOutAmount, totalBetAmount) {
            ...
            for (var i = 0; i < listOfRecords.length; ++i) {
                ...
                assert(user);
                var toAlloc = Math.min(bonusPool, maxWinRatio*listOfRecords[i].bet);//Math.round((listOfRecords[i].bet / totalBetAmount) * toAllocAll);
                if (listOfRecords[i].autoCashOut > crashPoint) {
                    toAlloc = 0;
                }
